...
from edoor.edoor.doctype.city_ledger_invoice.city_ledger_invoice import (
    update_city_ledger_invoice_balance,
)
from edoor.api.cache_functions import (
    get_account_code_doc,
    get_account_code_sub_account_information,
    get_cache_data,
)
from edoor.api.folio_transaction import update_reservation_folio
from edoor.api.generate_room_rate import (
    get_charge_breakdown_by_account_code_breakdown,
    get_room_rate_account_code_breakdown,
    package_base_account_code_charge_breakdown,
)
from edoor.api.tax_calculation import get_tax_breakdown
from edoor.api.utils import (
    add_audit_trail,
    get_working_day,
    update_city_ledger,
    update_deposit_ledger,
    update_desk_folio,
    update_payable_ledger,
)
...
def dome():
    data = {
        "name": "FT2024-3007",
        "owner": "Administrator",
        "creation": "2024-09-16 22:44:10.769546",
        "modified": "2024-09-16 22:44:10.769546",
        ...
        "posting_date": "2024-09-10T17:00:00.000Z",
        "working_day": "WD2024-0038",
        "cashier_shift": "CS2024-0125",
        "working_date": "2024-09-11",
        "reservation": "RS2024-0347",
        "reservation_stay": "ST2024-1927",
        "is_complimentary": 0,
        "is_house_use": 0,
        "transaction_type": "Reservation Folio",
        "transaction_number": "FN2024-0789",
        "naming_series": "FT.YYYY.-.####",
        "room_type": "Double room",
        "room_number": "A 102",
        "business_source": "Angkor leab",
        "business_source_type": "OTA",
        "room_type_id": "RT-0002",
        "room_type_alias": "DR",
        "room_id": "RM-0057",
        "reservation_type": "FIT",
        "adult": 1,
        "child": 0,
        "reservation_status_color": "#29CD42",
        "reservation_status": "In-house",
        "is_package": 0,
        "is_package_charge": 0,
        "is_package_breakdown": 0,
        "is_base_transaction": 1,
        "account_code": "10101",
        "account_name": "Room Charge",
        "type": "Debit",
        "input_amount": 200,
        "quantity": 1,
        "price": 178.25311943,
        ...
        "tax_2_amount": 0,
        "tax_3_amount": 0,
        "total_tax": 21.74688057,
        "bank_fee": 0,
        "bank_fee_amount": 0,
        "total_amount": 200,
        "parent_account_code": "10100",
        "parent_account_name": "Room Charge",
        "account_group": "10000",
        "account_group_name": "Charge",
        "account_category": "Room Charge",
        "account_category_sort_order": 200,
        "account_code_sort_order": 10101,
        "allow_enter_quantity": 0,
        "show_print_preview": 0,
        "is_night_audit_posting": 0,
        "flash_report_revenue_group": "Room Charge",
        "is_verify": 0,
        "discount_account": "40101",
        "discount_description": "Room Charge Discount - 0.0%",
        "tax_2_account": "20102",
        "tax_3_account": "20101",
        "reservation_room_rate": "a3138930-5111-4c33-b804-c9babaf843a7",
        "source_reservation_stay": "ST2024-1927",
        "stay_room_id": "aac7d2fca2",
        "report_description": "Room Charge",
        "is_auto_post": 1,
        "is_master_folio": 0,
        "is_housekeeping_account": 0,
...
def get_folio_transaction_calculation(folio_transaction_data=None):
    data = [] 
    folio_transaction_data["tax_rule"] = (
        ""
        if not "tax_rule" in folio_transaction_data
        else folio_transaction_data["tax_rule"]
    )
    folio_transaction_data["adult"] = (
        0 if not "adult" in folio_transaction_data else folio_transaction_data["adult"]
    )
    folio_transaction_data["child"] = (
        0 if not "child" in folio_transaction_data else folio_transaction_data["child"]
    )
        ...
        package_data = [
            {
                ...
                "discount_breakdown_account_code": d.discount_breakdown_account_code
                or "",
    ...
    account_codes = get_room_rate_account_code_breakdown(
        json.dumps(folio_transaction_data)
    )
    if folio_transaction_data["discount"] > 0:
        if folio_transaction_data["discount_type"] == "Amount":
            discountable_amount = sum(
                d["amount"] for d in account_codes if d["allow_discount"] == 1
            )
            folio_transaction_data["discount"] = folio_transaction_data["discount"] / (
                1 if discountable_amount == 0 else discountable_amount
            )

        else:
            # convert discount to decimal number 10%=0.1
            folio_transaction_data["discount"] = (
                folio_transaction_data["discount"] / 100
            )
    ...
    for acc in account_codes:
        ...
        tax_data["is_package"] = (
            0 if not "is_package_account" in acc else acc["is_package_account"]
        )
...
def create_folio_transaction(data):
    ...
    if "name" in data:
        ...
        working_day = {
            ...
            "cashier_shift": {"name": data["cashier_shift"]},
        }
    ...
    if "name" in data:
        ...
        frappe.db.sql(
            "update `tabFolio Transaction` set owner='{0}',creation='{1}' where name='{2}' or reference_folio_transaction='{2}'".format(
                data["owner"], data["creation"], data["name"]
            )
        )
    ...
    if data.get("city_ledger_invoice"):
        ...
        update_city_ledger_invoice_balance(
            data.get("city_ledger_invoice"), run_commit=False
        )
...
def validate_add_folio_transaction(data, working_day):
    ...
    if account_code_doc.target_document:
        ...
        if (
            not "target_transaction_number" in data
            or not data["target_transaction_number"]
        ):
            frappe.throw(_("Please select target transaction number"))

        if data.get("target_transaction_type") == "City Ledger":
            if (
                frappe.get_cached_value(
                    "City Ledger", data.get("target_transaction_number"), "status"
                )
                == "Closed"
            ):
                frappe.throw("This city ledger account is already closed")
    ...
    if not account_code_doc.allow_zero_amount_posting:
        ...
        if data["input_amount"] == 0:
            frappe.throw(
                _(
                    "Please enter amount for {account_name}".format(
    ...
    if account_code_doc.required_select_product:
        if not "items" in data:
            frappe.throw(
                _(
                    "Please select product for this account {account_code} - {account_name}".format(
        ...
        if [
            item for item in data["items"] if not item.get("product_code")
        ]:  # Check if product_code is empty or missing
            frappe.throw(
                _(
                    "Please input all products".format(
                        account_code=account_code_doc.name,
                        account_name=account_code_doc.account_name,
                    )
                )
    ...
    parent_doc_status = frappe.db.get_value(
        data["transaction_type"], data["transaction_number"], "status"
    )

    if parent_doc_status == "Closed":
        frappe.throw(
            _(
                "{transaction_type} number {transaction_number} is already closed.".format(
    ...
    if "reservation_stay" in data:
        reservation_status = frappe.db.get_value(
            "Reservation Stay", data["reservation_stay"], "reservation_status"
        )
        allow_edit_information = frappe.db.get_value(
            "Reservation Status", reservation_status, "allow_user_to_edit_information"
        )
        if not allow_edit_information:
            frappe.throw(
                _(
                    "This reservation stay {reservaiton_stay} with status {status} is not allow to edit information".format(
                        reservaiton_stay=data["reservation_stay"],
...
def get_folio_transaction_breakdown(data=None):
    ...
    rate_breakdown_param = {
        ...
        "discount_amount": (
            0 if not "discount_amount" in data else data["discount_amount"]
        ),
    ...
    account_code_breakdown = get_room_rate_account_code_breakdown(
        json.dumps(rate_breakdown_param)
    )

    account_code_breakdown = [
        d
        for d in account_code_breakdown
        if d["amount"] > 0 or d["is_package_account"] == 0
    ]
    account_code_breakdown = get_charge_breakdown_by_account_code_breakdown(
        json.dumps(account_code_breakdown)
    )
...
def add_folio_transaction_record(data, breakdown_data, working_day, old_doc=None):
    if "base_account" in breakdown_data:
        base_doc = get_folio_transaction_doc_share_property(
            data, breakdown_data["base_account"], working_day
        )
        if "name" in data:
        ...
        base_doc.transaction_amount = breakdown_data["base_account"][
            "transaction_amount"
        ]
        ...
        base_doc.price = (
            base_doc.amount
            if base_doc.quantity == 0
            else base_doc.amount / base_doc.quantity
        )
        base_doc.total_amount = (
            base_doc.amount
            - (base_doc.discount_amount or 0)
            + (base_doc.total_tax or 0)
        )
        ...
        if "package_accounts" in breakdown_data:
            base_doc.total_sub_package_charge = sum(
                [
                    d["amount"] - (d["discount_amount"] or 0) + (d["total_tax"] or 0)
                    for d in breakdown_data["package_accounts"]
                ]
            )
        ...
        if "bank_fee_account" in data:
            if "bank_fee" in data:
                base_doc.bank_fee_amount = base_doc.total_amount * (
                    data["bank_fee"] / 100
                )
                ...
                base_doc.transaction_amount = (
                    base_doc.transaction_amount + base_doc.bank_fee_amount
                )
            ...
            for item_data in data["items"]:
                base_doc.append(
                    "items",
                    {
                        ...
                        * (1 if item_data["quantity"] == 0 else item_data["quantity"]),
                    },
                )
            product_description = ", ".join(
                [
                    "{}{} - {}".format(
                        "" if d.quantity == 0 else str(d.quantity) + " x ",
                        d.product_name,
                        frappe.format(d.total_amount, {"fieldtype": "Currency"}),
                    )
                    for d in base_doc.items
                ]
            )
            base_doc.report_description = "{} {}".format(
                base_doc.account_name, "({})".format(product_description)
            )
        ...
        if base_doc.bank_fee_account:
            if base_doc.bank_fee:
                doc = get_folio_transaction_doc_share_property(
                    data,
                    {
                        "account_code": base_doc.bank_fee_account,
                        "amount": base_doc.bank_fee_amount,
                    },
                    working_day,
                )
                ...
                doc.report_description = "Bank Fee {bank_fee}% of {amount}".format(
                    bank_fee=base_doc.bank_fee,
                    amount=frappe.format_value(base_doc.input_amount, "Currency"),
                )
                doc.is_auto_post = 1
    ...
    if "package_accounts" in breakdown_data:
        for p in breakdown_data["package_accounts"]:
            ...
            doc.total_amount = (
                doc.amount - (doc.discount_amount or 0) + (doc.total_tax or 0)
            )
    ...
    if base_doc.target_transaction_type and base_doc.target_transaction_number:
        doc = get_folio_transaction_doc_share_property(
            data, {"account_code": base_doc.target_account_code}, working_day
        )
        doc.reference_number = base_doc.name
        doc.reference_folio_transaction = base_doc.name
        ...
        if data["target_transaction_type"] == "Reservation Folio":
            target_doc = frappe.get_doc(
                "Reservation Folio", data["target_transaction_number"]
            )
            ...
            doc.reservation_type = frappe.db.get_value(
                "Reservation", target_doc.reservation, "reservation_type"
            )
            doc.guest = target_doc.guest
            doc.guest_name = frappe.get_cached_value(
                "Customer", target_doc.guest, "customer_name_en"
            )
            doc.phone_number = frappe.get_cached_value(
                "Customer", target_doc.guest, "phone_number"
            )
            doc.email = frappe.get_cached_value(
                "Customer", target_doc.guest, "email_address"
            )

            doc.business_source = target_doc.business_source
            doc.business_source_type = frappe.get_cached_value(
                "Business Source", target_doc.business_source, "business_source_type"
            )
            doc.room_id = ""
        ...
        doc.city_ledger_invoice = city_ledger_invoice
        # prepare note
...
def get_folio_transaction_doc_share_property(data, folio_transaction_data, working_day):
    ...
    if doc.guest:
        guest_data = get_cache_data(
            "Customer", data["guest"], "customer_name_en,customer_group,country"
        )
        doc.guest_name = guest_data["customer_name_en"]
        doc.guest_type = guest_data["customer_group"]
        doc.nationality = guest_data["country"]

    account_info = get_account_code_sub_account_information(
        folio_transaction_data["account_code"]
    )
    ...
    if doc.reservation_stay:
        doc.source_reservation_stay = (
            doc.source_reservation_stay or doc.reservation_stay
        )
        # get reservation data  to update to folio transaction such as reservation type, reservation status, ....
        stay_info = get_cache_data(
            "Reservation Stay",
            doc.reservation_stay,
            "reservation_type,business_source,business_source_type,reservation_status,status_color",
        )
    ...
    if doc.target_transaction_type and doc.target_transaction_number:
        doc.city_ledger_name = frappe.get_cached_value(
            "City Ledger", doc.target_transaction_number, "city_ledger_name"
        )
        doc.city_ledger_type = frappe.get_cached_value(
            "City Ledger", doc.target_transaction_number, "city_ledger_type"
        )
    ...
    if "discount_account" in account_info:
        ...
        if doc.discount_account:
            if data["discount_type"] == "Percent":
                doc.discount_description = "{} - {}%".format(
                    account_info["discount_account_name"], data["discount"]
                )  # Room Charge Discount - 50.0%
            else:
                doc.discount_description = account_info["discount_account_name"]
            doc.discount_type = (
                "Percent" if not "discount_type" in data else data["discount_type"]
            )
    ...
    if "tax_1_account" in account_info and "tax_1_rate" in folio_transaction_data:
        ...
        if folio_transaction_data["tax_1_rate"] > 0:
            ...
            doc.tax_1_description = "{} - {}%".format(
                account_info["tax_1_description"], folio_transaction_data["tax_1_rate"]
            )
    ...
    if "tax_2_account" in account_info and "tax_2_rate" in folio_transaction_data:
        ...
        if folio_transaction_data["tax_2_rate"] > 0:
            doc.tax_2_description = "{} - {}%".format(
                account_info["tax_2_description"], folio_transaction_data["tax_2_rate"]
            )
    ...
    if "tax_3_account" in account_info and "tax_3_rate" in folio_transaction_data:
        ...
        if folio_transaction_data["tax_3_rate"] > 0:
            doc.tax_3_description = "{} - {}%".format(
                account_info["tax_3_description"], folio_transaction_data["tax_3_rate"]
            )
...
def update_room_information(doc):
    room_info = frappe.db.sql(
        "select room_id,room_number,room_type,room_type_id, room_type_alias from `tabRoom Occupy` where reservation_stay=%(reservation_stay)s and property=%(property)s and date=%(date)s limit 1",
        {
            "property": doc.property,
            "reservation_stay": (doc.source_reservation_stay or doc.reservation_stay),
    ...
    if room_info:
        ...
        room_info = frappe.db.sql(
            "select room_id,room_number,room_type,room_type_id, room_type_alias from `tabReservation Stay Room` where parent=%(reservation_stay)s and property=%(property)s  limit 1",
            {"property": doc.property, "reservation_stay": doc.reservation_stay},
            as_dict=1,
        )
        if room_info:
            room_info = room_info[0]
    ...
    if doc.target_transaction_type and doc.target_transaction_number:
        ...
        if doc.target_transaction_type == "Reservation Folio":
            room_number = frappe.db.get_value(
                "Reservation Folio", doc.target_transaction_number, "rooms"
            )
            doc.note = (doc.note or "") + _(
                "Folio balance transfer to folio # {folio_number}, room: {room_number} ".format(
                    folio_number=doc.target_transaction_number, room_number=room_number
                )
            )
        elif doc.target_transaction_type == "City Ledger":
            doc.note = (doc.note or "") + _(
                "City Ledger Transfer to {city_ledger} - {city_ledger_name}".format(
                    city_ledger=doc.target_transaction_number,
                    city_ledger_name=doc.city_ledger_name,
                )
            )
        ...
        if doc.note:
            doc.note = doc.note + "\n"
        doc.note = (doc.note or "") + _(
            "Transfer from {source_transaction_type} #: {source_transaction_number}, Room: {room_number}".format(
                transaction_type=doc.transaction_type,
                source_transaction_type=doc.source_transaction_type,
                source_transaction_number=doc.source_transaction_number,
                reference_folio_transaction=doc.reference_folio_transaction,
                room_number=base_doc.room_number,
            )
        )
...
def update_transaction_type_summary(data):
    ...
    if data["transaction_type"] == "Reservation Folio":
        update_reservation_folio(
            name=data["transaction_number"], run_commit=False, ignore_validate=True
        )

    elif data["transaction_type"] == "Deposit Ledger":
        update_deposit_ledger(
            name=data["transaction_number"],
            run_commit=False,
...
def add_audit_trail_when_edit_transaction(new_doc, old_doc, working_day):
    ...
    for field in new_doc.meta.fields:
        ...
        if value1 != value2:
            changes[fieldname] = {"old_value": value2, "new_value": value1}
    audit_trail_document = frappe.get_cached_doc(
        "Audit Trail Document", "Folio Transaction"
    )
    ...
    change_string = change_string + ", ".join(
        [
            ...
            if key in [x.field_name for x in audit_trail_document.tracking_field]
        ]
    )
...
def delete_transaction(parent_transaction_name):
    frappe.db.sql(
        "delete from `tabFolio Transaction` where reference_folio_transaction='{0}' or name='{0}'".format(
            parent_transaction_name
        )
    )
    frappe.db.sql(
        "delete from `tabFolio Transaction Products` where parent='{0}'".format(
            parent_transaction_name
        )
    )
    ...
    # update tab series
    frappe.db.sql(
        "update  `tabSeries` set current = 0 where name = '{}-'".format(
            parent_transaction_name
        )
    )
