...
class RoomBlock(Document):
	...
	def before_submit(self):
		...
		sql = sql.format(self.room_id, self.start_date, add_to_date(getdate(self.end_date),days=-1), self.property)
	...
	def on_update_after_submit(self):
		if self.is_unblock ==1:
			...
			if self.end_date != old_doc.end_date:
				...
				sql = "select name, date,type from `tabTemp Room Occupy` where room_id='{}' and date between '{}' and '{}' and property='{}' and stay_room_id != '{}' limit 1 "
				sql = sql.format(self.room_id, self.start_date, add_to_date(getdate(self.end_date),days=-1), self.property,self.name)
				data = frappe.db.sql(sql, as_dict=1)
				if data:
					frappe.throw("Room {} is already occupy or block on date {}".format( self.room_number,data[0]["date"].strftime("%d-%m-%Y")))
...
def generate_block_date(self):
	...
	frappe.db.sql("delete from `tabTemp Room Occupy` where stay_room_id='{}'".format(self.name))
	frappe.db.sql("delete from `tabRoom Occupy` where stay_room_id='{}'".format(self.name))
