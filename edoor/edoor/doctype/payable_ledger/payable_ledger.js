...
frappe.ui.form.on("Payable Ledger", {
    refresh(frm) {
      set_indicator(frm);
  ...
  function getItemListFromGuestFolio(frm) {
    ...
    frappe.db
      ...
      .then((result) => {
        ...
        let html = frappe.render_template("payable_ledger_transaction_list", {
    ...
    frappe
      ...
      .then((result) => {
        html = frappe.render_template("payable_ledger_summary_list", {
          data: result.message,
        });
