...
def execute(filters=None):
    report_config = frappe.get_last_doc(
        ...
        filters={"property": filters.property, "report": "Arrival Guest Report"},
    )
...
def get_report_columns(filters, report_fields):
    ...
    if filters.show_columns:
        report_fields = [
            d for d in report_fields if d.fieldname in filters.show_columns
        ]
...
def get_report_data(filters, report_fields):
    ...
    if filters.show_in_group_by:
        ...
        for parent in parent_row:
            ...
            report_data.append(
                {"indent": 0, report_fields[0].fieldname: d, "is_group": 1}
            )

            report_data = report_data + [
                d for d in data if d[filters.show_in_group_by] == parent
            ]
...
def get_data(filters, report_fields):
    sql = "select {} as indent, 0 as is_group, ".format(
        1 if filters.show_in_group_by else 0
    )

    sql = "{} {}".format(
        sql, ",".join([d.sql_expression for d in report_fields if d.sql_expression])
    )

    if (
        filters.show_in_group_by
        and len([d for d in report_fields if d.fieldname == filters.show_in_group_by])
        == 0
    ):
...
def get_filters(filters):
    ...
    if filters.room_types:
        sql = """{} and  name in (
            ...
            sql, get_room_rate_filters(filters)
        )
    return sql
...
def get_report_summary(filters, report_fields, data):
    ...
    if filters.show_in_summary:
        summary_fields = [
            d for d in summary_fields if d.fieldname in filters.show_in_summary
        ]
        ...
        summary.append(
            {
                "value": sum(
                    [
                        d[x.fieldname]
                        for d in data
                        if d["is_group"] == 0 and x.fieldname in d
                    ]
                ),
