...
def execute(filters=None):
	report_config = frappe.get_last_doc("Report Configuration", filters={"property":filters.property, "report":"Room Charge Posting Report"} )
...
def get_data (filters,report_fields):
	...
	sql = "{} from `tabFolio Transaction` rst ".format(sql)
...
def get_filters(filters):
	sql = """where property=%(property)s  and is_auto_post = 1 and posting_date = %(start_date)s and 
