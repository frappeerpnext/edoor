    ...
    for c in [d for d in columns if "has_total" in d ]:    
        ...
        if "date_index" in c:
            if(c.get("date_index")<len(total_rooms)):
                total_room =  sum([d.get("total_room") for d  in total_rooms if d.get("date").day ==  c.get("date_index") + 1 ])
                occupancy =  100 * ( (occupy or 0) / max(total_room,1))
                total_row[c["fieldname"]] = round(occupancy,0)
    
    # occupy
    occupy = sum([d["total"] for d in data])
    total_row["total"] = round( 100 *(  occupy / max(sum([d["total_room"] for d in total_rooms]),1)),0)
    return total_row
...
def get_grand_total_occupancy_row(data, columns,total_rooms):
    ...
    for c in [d for d in columns if "has_total" in d ]:
        if "date_index" in c:
            ...
            occupancy = round( occupy /  max(total_room,1) * 100,0)
            row[c["fieldname"]] =occupancy
 
    occupy = sum([d["total"] for d in data])
    row["total"] = round( 100 *(  occupy / max(sum([d["total_room"] for d in total_rooms]),1)),0)
    return row
