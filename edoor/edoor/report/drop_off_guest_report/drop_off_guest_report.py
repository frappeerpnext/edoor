...
def execute(filters=None):
	report_config = frappe.get_last_doc("Report Configuration", filters={"property":filters.property, "report":"Drop-off Guest Report"} )
...
def get_filters(filters):
	sql = "where property=%(property)s  and departure_date between %(start_date)s and %(end_date)s and is_active_reservation = 1 and require_drop_off=1 "
