...
def get_folio_transaction_data(filters,indent):
	sql = """select 
				...
				sum(amount*if(type='Debit',1,-1)) as net_total
			from `tabFolio Transaction`
			where 
				property = %(property)s and 
	...
	sql = sql.format(
		indent, 
		"transaction_type," if filters.group_by_ledger_type==1 else "", 
		"account_name" if not filters.show_account_code  else "concat(account_code,'-',account_name)",
		filter
	)
