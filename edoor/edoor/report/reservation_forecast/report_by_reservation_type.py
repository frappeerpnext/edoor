...
def get_report_columns(filters,report_config):
    ...
    columns = [
        {'key': "Reservation Type","fieldname":"row_group","label":"Reservation Type","width":50},
    ]
...
def get_occupy_data(filters,report_config):
    sql = "select reservation_type as row_group, "
    ...
    sql = "{} and ifnull(reservation_type,'') !='' ".format(sql)


    # group by
    
    sql = "{} group by reservation_type".format(sql)
