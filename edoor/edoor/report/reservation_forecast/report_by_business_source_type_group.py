...
import copy
from edoor.edoor.report.reservation_forecast.utils import get_report_chart,get_report_summary,get_room_rate_filters,get_occupy_data_filters,get_parent_group_by_record,get_room_occupy_group_by_field,get_room_rate_group_by_field,get_parent_group_row_from_result_data,get_parent_row_group_label,get_report_fields,get_row_group_from_result_data
...
def get_report_columns(filters,report_config):
    ...
    columns = [
        {'key': "Business Source Group","fieldname":"row_group","label":"Business Source Group","width":150},
    ]
...
def get_report_data(filters,report_config):
    ...
    report_group_data = get_row_group_from_result_data(data, room_rate_data)
    ...
    for parent in parent_row_group_data:
        ...
        for row in sub_report_data:
            ...
            if filters.parent_row_group and filters.parent_row_group in ["Date","Month","Year", "Room Type"]:
                ...
                row["room_block"] = sum([d["room_block"] for d in room_block_data])
...
def get_occupy_data(filters,report_config):
    sql = "select bt.business_source_group as row_group, "
    ...
    #filter
    sql = sql+ """ from `tabRoom Occupy` a  
                    JOIN `tabBusiness Source Type` bt on bt.name = a.business_source_type
            where 1=1 
        """
    ...
    sql = "{} and ifnull(bt.business_source_group,'') !='' ".format(sql)


    # group by
    
    sql = "{} group by bt.business_source_group".format(sql)
...
def get_room_available(filters):
    ...
    if filters.parent_row_group in ["Date","Month","Year","Room Type"]:
        sql = "{} group by  {}".format(sql,format(get_room_occupy_group_by_field(filters)))
...
def get_room_block_data(filters):
    ...
    if filters.room_type:
        sql = "{} and room_type_id=%(room_type)s ".format(sql)
    if filters.parent_row_group in ["Date","Month","Year","Room Type"]:
        sql = sql  + " group by " + get_room_occupy_group_by_field(filters)
...
def get_room_rate_data(filters, report_config ):
    ...
    #filter
    sql = sql+ """ from `tabRevenue Forecast Breakdown` a 
                join `tabBusiness Source Type` bt on bt.name = a.business_source_type
                where 1=1 """
