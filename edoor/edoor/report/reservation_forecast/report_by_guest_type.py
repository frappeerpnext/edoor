...
def get_report_columns(filters,report_config):
    ...
    columns = [
        {'key': "Guest Type","fieldname":"row_group","label":"Guest Type","width":150},
    ]
...
def get_occupy_data(filters,report_config):
    sql = "select guest_type as row_group, "
    ...
    sql = "{} and ifnull(guest_type,'') !='' ".format(sql)


    # group by
    
    sql = "{} group by guest_type".format(sql)
