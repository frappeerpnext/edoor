<template>
    <DataTable :value="data">
        ...
        <Column field="room_type_alias" :header="$t('Room Type')" />
        ...
        <ColumnGroup type="footer">
            <Row>
                ...
                <Column footerStyle="text-align:center">
                    <template #footer>
                        {{ (getTotal('total_room_sold') / getTotal('occupancy_room') * 100).toFixed(2) }}%
                    </template>
                </Column>
...
onMounted(() => {
    setTimeout(() => {
        loading.value = true
    getApi("frontdesk.get_daily_summary_by_room_type", {
