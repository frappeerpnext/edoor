<template>
    ...
    <template #top-summary>
        <div class="my-4">
        <ComLedgerBalanceKPI ledgerType="Reservation Folio" :startDate="working_date" :endDate="working_date">
    </ComLedgerBalanceKPI>
</div>
    </template>
    <template #name="{ item, index }">
            <Button class="link_line_action1" @click="onOpenLink('view_reservation_folio_detail', item.name)" link>
                {{ item.name }}
            </Button>
        </template>
