    ...
    <ComDialogContent @onOK="onSave" :loading="isSaving" hideButtonClose>
        <div class="mb-3 flex overflow-auto">
            ...
            <span class="con" @click="onViewReservationStayDetail(doc?.name)">
                <ComTagReservation title="RES STAY#:" :value="doc?.name" class="link_line_action w-auto">
                </ComTagReservation>
            </span>
            <div v-tippy="'Split Room'" class="flex justify-center items-center px-2 rounded-lg me-2 bg-card-info p-1px"
                v-if="doc?.stays?.length >= 2">
                <ComIcon icon="iconSplit" style="height:15px;"></ComIcon>
            </div>
            <span class="px-2 rounded-lg me-2 text-white p-1px"
                :style="{ background: doc?.status_color }">{{ doc?.reservation_status }}</span>
        </div>
        <div class="grid">
            <div class="col-6">
                <table>
                    ...
                    <ComStayInfoNoBox label="Guest name">
                        <div class="inline -ml-3">
                            <span @click="onViewGuestDetail(doc?.guest)" class="link_line_action">{{ doc?.guest_name
                            }}</span>
                        </div>
                    </ComStayInfoNoBox>
                    ...
                    <ComStayInfoNoBox label="Reservation Type" :value="doc?.reservation_type">
                        <span v-tippy="'Res Color'" v-if="doc?.reservation_color"
                            :style="{ background: doc?.reservation_color }" class="px-3 rounded-lg font-semibold">&nbsp;</span>
                    </ComStayInfoNoBox>
                    <ComStayInfoNoBox v-if="doc?.reservation_type == 'GIT'" label="Group">
                        <div class="flex font-semibold text-right -ml-3">
                            <sapn v-if="doc?.group_code" v-tippy="'Group Code'">{{ doc?.group_code }}</sapn><span
                                v-if="doc?.group_code && doc?.group_name">/</span> <span v-if="doc?.group_name"
                ...
                <ComStayInfoNoBox v-if="doc?.stays.length > 1" label="Rooms">
                    <div v-if="doc && doc?.stays">
                        <div class="-ml-3">
                            ...
                            <div v-if="doc?.stays.length > 3" v-tippy="{
                                value: `<div class='tooltip-room-stay'> ${doc?.stays.stays.slice(3).map(obj => obj.room_types + '/' + (obj.room_number || '')).join('\n')}</div>`
                    ...
                    <ComStayInfoNoBox label="Rooms">
                        <div v-if="doc && doc?.stays">
                            <div class="-ml-3">
                                <div class="rounded-xl px-2 me-1 bg-gray-edoor inline">
                                    <span
                                        v-tippy="data?.extendedProps?.room_type">{{ data?.extendedProps?.room_type_alias }}</span>
                    ...
                    <ComStayInfoNoBox label="Rooms">
                        <div class="inline -ml-3 ">
                            <template v-if="new_data">
                                <div class="rounded-xl px-2 me-1 bg-gray-edoor inline">
                                    <sapn v-tippy="new_data?.extendedProps?.room_type">{{
                                        new_data?.extendedProps?.room_type_alias }}</sapn> / <span>{{ new_data?.title
    }}</span>
                                </div>
                                ...
                                <div v-if="doc && doc?.stays">
                                    <div class="">
                                        <div class="rounded-xl px-2 me-1 bg-gray-edoor inline">
                                            <span
                                                v-tippy="data?.extendedProps?.room_type">{{ data?.extendedProps?.room_type_alias }}</span>
                                            <span v-if="data?.extendedProps?.room_number"> / {{
                                                data?.extendedProps?.room_number }} </span>
                                        </div>
                                    </div>
                                </div>
                            </template>
