...
const calendarOptions = reactive({
    ...
    eventClick: ((info) => {
        ...
        if(info.event._def.extendedProps.type=="room_type_event" || info.event._def.extendedProps.type=="room_inventory_room_type_summary" ){
            onViewRoomOccupy(info)
        }else {
...
function onViewRoomOccupy(event){
    
    const data = event.event._def
  
    dialog.open(ComIFrameModal, {
        data: {
            "doctype": "Business Branch",
            name: window.property_name,
            report_name: gv.getCustomPrintFormat("eDoor Room Occupy List"),
            view: "ui",
            room_type:data.extendedProps.room_type,
            extra_params: [{key:"date",value:moment(data.date).format("YYYY-MM-DD")}],
            filter_options: ['keyword', 'business_source', 'room_type', 'reservation_status'],
            fullheight: true
        },
        props: {
            header: $t("Room Occupy on " + " - " + moment(data.date).format("DD-MM-YYYY")),
            style: {
                width: '90vw',
            },
            position: "top",
                ...
                if (r.id == "vacant_room") {
                  ...
                  event.color="rgb(239 237 234 / 67%)"
                   ...
                   event.color="rgb(239 237 234 / 67%)"
                    ...
                    event.title =  current_date_occupy?.total || 0
