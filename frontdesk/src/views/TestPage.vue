        ...
        <div style="max-width: 100%; height: 1000px">
            <div>
                <div class="relative" aria-haspopup="true" aria-controls="overlay_menu">
                    ...
                    <p>Filter date need option run night audit</p>
...
function getRoomChartlocationStorage() {
    if (localStorage.getItem('reservation_chart')) {
        ...
        return setRoomChartlocationStorage()
    }
}
function setRoomChartlocationStorage(data = {}) {
    ...
    const dataStorage = {
        view: data.view || 'room_type',
        peroid: data.peroid || 'today',
        date: _date
    }
...
function onInitialDate(date = '', refresh = false) {
    ...
    let roomChartStartDate = ''
    if (date) {

        //localStorage.setItem('reservation_chart_date', date)
        const storage = setRoomChartlocationStorage({
            date: date
        })
        roomChartStartDate = storage.date
    }
    else {
        const roomChartStorage = getRoomChartlocationStorage()
        const view_date = roomChartStorage.date //localStorage.getItem('reservation_chart_date')
        if (refresh || !view_date) {
            const date_working_day = moment(working_day.date_working_day).add(-1, 'days').format("yyyy-MM-DD")
            //localStorage.setItem('reservation_chart_date', date_working_day)
            const storage = setRoomChartlocationStorage({
                date: date_working_day
            })
            roomChartStartDate = date_working_day
            return date_working_day
        } else {
            roomChartStartDate = view_date
            return view_date
        }
    }
    console.log(roomChartStartDate)
    onUpdateFilterDate(roomChartStartDate)
...
function onFilter(key, refresh = false, filter_date = '') {
    ...
    if (key == 'today') {
        cal.changeView('resourceTimeline', { start: onInitialDate('', true), end: moment(onInitialDate('', true)).add(1, 'months').format("yyyy-MM-DD") });
    }
    else if (key == 'week') {
...
function onPrevNext(key) {
    ...
    if (key == 'prev') {
        ...
        onInitialDate(moment(startDate).add(1, 'days').format("yyyy-MM-DD"))
