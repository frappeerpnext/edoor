<template> 
<div v-if="floorTemplate.template.room_reservation == 'template_1'">
    <div class="w-full h-full  p-2  relative">
     ...
     <div class="text-md  font-medium">
      {{ room?.room_type_alias }} -  {{ room.room_number }}</div>
     <div class="w-full text-overflow-ellipsis floor-plan-guest-name">
      {{ stay.guest_name }}
     </div>
     <div class="mt-1">
...
<div v-if="stay?.group_color" v-tippy="{ content: stay.group_name || group_code ? stay.group_name + ' - ' + stay.group_code || '' :  'Group Color', placement: 'left' }" class="absolute right-0 bottom-0 overflow-hiddent" :style="{ backgroundColor: stay?.group_color }" style="clip-path: circle(99.3% at 100% 100%);width:20px;height: 20px;"></div>
    ...
    <!-- template 2 -->
    <div v-else-if="floorTemplate.template.room_reservation == 'template_2'">
    <div class="w-full h-full  p-2  relative" >
        <div class="line-height-1 px-2" >
     <div class="text-md font-medium" >
      {{ room?.room_type_alias }} -  {{ room.room_number }}</div>
...
const floorTemplate = ref(JSON.parse(window.setting.floor_plan_template));
