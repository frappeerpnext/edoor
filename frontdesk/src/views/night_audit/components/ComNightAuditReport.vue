    ...
    <div> 
        ...
        <Splitter class="mb-5" state-key="report_spliter_state" state-storage="local">
            <SplitterPanel :size="25" class="pa-4">
                <div class="p-2">
                    <Button :class="{ active: activeButton === index }" class="night-audit-report-btn" @click="onViewReport(d, activeButton=index)" v-for="(d, index) in report_list" :key="index"> 
                        {{ d.report_title }}
                    </Button>
                </div>
            </SplitterPanel>
            ...
            <SplitterPanel :size="75" class="pa-4">
                <div v-if="selectedReport" class="p-2">
                    ...
                    <iframe @load="onIframeLoaded()" style="height:700px;" id="iframe_night_audit_report" width="100%" :src="url"></iframe>
                </div>
