...
function viewFolioSummaryReport() {
    dialog.open(ComIFrameModal, {
        data: {
            ...
            report_name: gv.getCustomPrintFormat("eDoor Payable Ledger Invoice Summary"),
            show_letter_head: true,
            filter_options:['invoice_style']
        },
        props: {
            header: "Payable Ledger Invoice Summary",
            style: {
                width: '80vw',
            },
            position: "top",
...
print_menus.value.push({
    label: "Payable Ledger Summary Report",
    icon: 'pi pi-print',
    command: () => {
    ...
    command: () => {
        dialog.open(ComIFrameModal, {
            data: {
                doctype: "Payable%20Ledger",
                name: selectedFolio.value.name,
                report_name: gv.getCustomPrintFormat("eDoor Payable Ledger Invoice Detail"),
                show_letter_head: true,
...
function EditFolio() {
    ...
    const dialogRef = dialog.open(ComAddPayableLedger, {

        data: {
            name:selectedFolio.value.name,
        },
        props: {
            header: 'Edit Payable Ledger' + selectedFolio.value.name,
            style: {
                width: '50vw',
            },
            modal: true,
...
function openFolio() {
        ...
        confirm.require({
        header: 'Open Payable Ledger ' + selectedFolio.value.name,
        message: 'Are you sure you want to open this payable ledger' + selectedFolio.value.name + '?',
        ...
        rejectClass: 'hidden',
        accept: () => {
            updateDoc('Payable Ledger', selectedFolio.value.name, {
                status: 'Open',
            })
                .then((doc) => {
...
function closeFolio() {
    confirm.require({
        header: 'Close Payable Ledger ' + selectedFolio.value.name,
        message: 'Are you sure you want to close this payable ledger' + selectedFolio.value.name + '?',
        icon: 'pi pi-info-circle',
        acceptClass: 'border-none crfm-dialog',
        acceptIcon: 'pi pi-check-circle',
        acceptLabel: 'Ok',
...
function onDeleteFolio() {
    ...
    const dialogRef = dialog.open(ComDialogNote, {
        data: {
            ...
            data: { doctype: "Payable Ledger", name: selectedFolio.value.name },
        },
        props: {
            header: "Delete Payable Ledger" + " " + selectedFolio.value.name,
            style: {
                width: '50vw',
            },
            modal: true,
