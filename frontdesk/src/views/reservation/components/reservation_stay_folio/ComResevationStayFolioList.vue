<template>
  <div
    class="col-fixed relative pl-0 pr-0 py-0"
    :style="`width: ${panelWidth ? panelWidth : '250px'}`"
  >
    <div
      class="flex flex-column justify-content-between h-full res-stay-folio-btn-site-bg"
    >
      <div
        :style="
          rs.is_page == true ? 'margin-bottom: 1px;' : 'margin-bottom: 60px;'
        "
      >
        <div class="flex justify-content-between align-items-center p-2">
          ...
          <Button
            class="btn-add-folio"
            @click="onAddCreatNewFolio"
            v-tippy="$t('Create Folio')"
          >
            <img
              :src="theme == 'estc' ? plus_svg : iconPlusSignWhite"
              style="height: 13px"
            />
          </Button>
        </div>
        <template v-if="rs.folios && rs.folios.length > 0">
          <Button
            :severity="item.selected ? 'folio-active' : 'primary'"
            v-for="(item, index) in Enumerable.from(rs.folios)
              .orderByDescending('$.is_master')
              .toArray()"
            :key="index"
            @click="onClick(item)"
            ...
            style="z-index: 1"
          >
            <span v-if="item.is_master == 1"
              ><img
                :src="crown_svg"
                class="folio-is-mater-icon me-2"
                style="height: 13px"
            /></span>
            <span class="flex flex-column align-items-start w-full">
              <span class="flex justify-content-between w-full">
                <span class="line-height-2">
                  ...
                  <span v-if="item.mark_as_verified == 1" class="text-blue-500 mt-1">
              ...
              <span class="flex justify-content-between w-full mt-1">
                <div class="flex text-left line-height-2">
                  <span
                    ...
                    v-tippy="item.guest_name"
                    class="white-space-nowrap text-gray-600 line-height-1 py-1 text-sm folio-remark cus me-1 overflow-hidden text-overflow-ellipsis"
                  >
                    <img
                      :src="guest_svg"
                      style="
                  ...
                  <span
                    v-tippy="'Folio Type'"
                    :style="{ backgroundColor: item.folio_type_color }"
                    v-if="item.folio_type"
                    class="white-space-nowrap text-white line-height-1 py-1 text-sm folio-remark cus me-1 overflow-hidden text-overflow-ellipsis"
      ...
      <div
        ...
        :class="
          rs.is_page == true
            ? 'flex flex-column bg-white mt-3 page_total_foliolist'
            : 'flex flex-column bg-white mt-3 relative lg:fixed total_foliolist'
        "
        :style="`width: ${
          panelWidth ? panelWidth : '250px'
        };bottom:0px;z-index: 1;`"
      >
        <div
          class="flex justify-content-end align-items-cente border-1 border-red-100 p-2"
        >
          <div class="pr-3">
            <label> {{ $t("Total Debit") }} </label>
          </div>
          <div>
            <span>
              <CurrencyFormat
                :value="rs?.reservationStay?.total_debit"
                class="white-space-nowrap font-medium"
              />
            </span>
          </div>
        </div>
        ...
        <div
          class="flex justify-content-end align-items-cente border-1 border-red-100 border-top-none p-2"
        >
          <div class="pr-3">
            <label>{{ $t("Total Credit") }}</label>
          </div>
          <div>
            <span>
              <CurrencyFormat
                :value="rs?.reservationStay?.total_credit"
                class="white-space-nowrap font-medium"
              />
            </span>
          </div>
        ...
        <div
          class="flex justify-content-end align-items-center border-1 border-red-100 border-top-none p-2"
        >
          <div class="pr-3">
          ...
          <div>
            <span>
              <CurrencyFormat
                :value="rs?.reservationStay?.balance"
                class="white-space-nowrap font-medium"
              />
            </span>
          </div>
