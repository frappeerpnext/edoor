<template>
    <div class="col-fixed pl-0 pr-0 py-0" style="width: 250px;">
        ...
        <div class="flex flex-column justify-content-between h-full res-stay-folio-btn-site-bg">
            <div>
                ...
                <template v-if="rs.folios && rs.folios.length > 0">
                    <Button :severity="item.name == rs.selectedFolio.name ? 'folio-active' : 'primary'"
                        v-for="(item, index) in Enumerable.from(rs.folios).orderByDescending('$.is_master')" :key="index" @click="onClick(item)"
                        ...
                        <span class="flex flex-column align-items-start w-full">
                            ...
                            <span class="flex text-left line-height-2">
                                <span style="display:inline-block;max-width: 165px;" v-tooltip="item.guest_name"
                                    class="white-space-nowrap text-gray-600 line-height-2 font-italic text-sm folio-remark cus me-1 overflow-hidden text-overflow-ellipsis">
            ...
            <div class="flex flex-column bg-white  mt-3">
                <div class="flex justify-content-end align-items-cente border-1 border-red-100 p-2">
                    <div class="pr-3"><label>Total</label></div> 
                    <div><span><CurrencyFormat value="100" class="white-space-nowrap font-medium" /></span></div>
                </div>
