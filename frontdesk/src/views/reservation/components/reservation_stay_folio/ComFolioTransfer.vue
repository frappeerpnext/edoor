...
<div class="grid">
    ...
    <ComReservationStayPanel class="h-full" title="Transferring To">
        <template #content>
            ...
            <Message v-else :closable="false" severity="success" icon="pi pi-exclamation-circle">
                <div class="flex align-items-center"> You Are Select {{ selectedFolio?.value  }} - {{ selectedFolio?.description }}</div>
            </Message>
    <label>Folio Number</label>
    <ComAutoComplete @onSelected="onSelectFolioNumber" v-model="data.new_folio_number" placeholder="Select Folio"
        doctype="Reservation Folio" class="auto__Com_Cus w-full"
        :filters="{ 'property': property_name , status: 'Open', 'name': ['!=', data.folio_number] }" />
        <label>Note</label>
    ...
    <div class="col-12">
        ...
        <template #content>
            <DataTable :rowClass="rowClass" class="p-datatable-sm mt-2" :value="data.folio_transaction"  tableStyle="">
