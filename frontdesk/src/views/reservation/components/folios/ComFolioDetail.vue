                ...
                <div v-if="doc" class="mt-2">
                    ...
                    <div class="py-2 mt-1 border-1 bg-slate-200 font-medium text-start ps-3 w-full">
                        <div class="flex gap-2 align-items-center">
                            {{ $t('Folio Detail') }} - {{ doc.name }} <span class="ms-2 px-2 rounded-lg  text-white p-1px"
                                :class="doc.status == 'Open' ? 'bg-green-500' : 'surface-500'">{{ $t(doc.status)  }}</span>
                            <div v-tippy="$t('Master Folio')" v-if="doc.is_master"
