<template>
    <ul>
        <template v-for="(s, index) in rs.reservationFolioList" :key="index"> 
            <li>
                {{ s.name }} | {{ s.guest_name }} | {{ s.balance }} {{ s.reservation_status }} | {{ s.status_color }}       | {{ s.rooms }}
                <ul>
                    <li v-for="(d, index) in s.folios" :key="index">
                        <Button @click="onSelectFolio(d)" v-if="d.selected" style="background-color: red;">
                            {{ d.name }} | {{ d.posting_date }} | {{ d.guest_name }} | {{ d.balance }} | selected :{{ d.selected }} | Is Master:{{ d.is_master }} <br/>
                            Balance:{{ d.balance }}
                        </Button>
                        <Button @click="onSelectFolio(d)" v-else>
                            {{ d.name }} | {{ d.posting_date }} | {{ d.guest_name }} | {{ d.balance }} | selected :{{ d.selected }} | Is Master:{{ d.is_master }}
                            <br/>
                            Balance:{{ d.balance }} 
                            Status: {{d.status}}
                        </Button>
...
const dialog = useDialog();
    ...
    //this method is in ComReservationFolio.vue
