    ...
    <div v-if="doc" class="mt-2">
      ...
      <div class="grid" style="margin-top: 10px">
        <div class="col">
          <table>
            <tbody>
              <tr>
                <th
                  class="py-2 mt-1 border-1 bg-slate-200 font-medium text-start ps-3"
                  colspan="2"
                >
                  {{ $t("City Ledger Invoice Information") }} -
                  <ComStatus :status="doc.status" />
                  <ComStatus :status="doc.payment_status" />
                </th>
              </tr>

              <ComStayInfoNoBox label="Invoice #" :value="doc?.name" />
              <ComStayInfoNoBox
                label="Reference #"
              ...
              <ComStayInfoNoBox label="City Ledger">
                <Stack :row="true">
                  <span
                    @click="OnViewCityLedgerDetail"
                    v-tippy="'Click to view city ledger detail'"
                    class="link_line_action1"
                    style="margin-left: -10px"
                    >{{ doc?.city_ledger }}</span
                  >
                  <span v-tippy="'City Ledger Name'">
                    {{ doc?.city_ledger_name }},</span
                  >
                  <span v-tippy="'Contact Name'" v-if="doc?.contact_name">
                    {{ doc?.contact_name }},</span
                  >
                  <span v-tippy="'Phone Number'" v-if="doc?.phone_number">
                    {{ doc?.phone_number }}</span
                  >
                </Stack>
              </ComStayInfoNoBox>
              <ComStayInfoNoBox label="Posting Date">
                <span class="font-normal text-right -ms-3">
                  {{ moment.utc(doc?.posting_date).format("DD-MM-YYYY") }}
                </span>
              </ComStayInfoNoBox>
              <ComStayInfoNoBox label="Payment Status">
                <span class="-ms-3 text-right">
                  {{ doc?.payment_status }}
                </span>
              </ComStayInfoNoBox>

              <ComStayInfoNoBox label="Created By">
                <span @click="" class="-ms-3 text-right font-italic">
                  {{ doc?.owner }},
                  <ComTimeago :date="doc?.creation" />
                </span>
              </ComStayInfoNoBox>
              <ComStayInfoNoBox label="Modified By">
                <span class="font-italic text-right -ms-3">
                  {{ doc?.modified_by }},
        ...
        <div class="col">
          ...
          <div>
            <div
              v-for="(item, index) in balance_doc"
              :key="index"
              class="flex mt-2 gap-2"
            >
              <ComBoxStayInformation
                :title="item?.account_group_name || 'Undefine'"
                :value="item?.total_amount || 0"
                isCurrency
                valueClass="col-6 text-right bg-gray-edoor-10 font-semibold"
      ...
      <Stack>
        ...
        >
              ...
              <div>
                ...
                <div>
                  ...
                  <span
                    ...
                    @click="
                      onOpenLink('view_reservation_detail', item.reservation)
                    "
                    class="text-blue-300 cursor-pointer w-auto"
                  >
                    {{ item.reservation }}
                  </span>
                  | ST:
          ...
          <template #name="{ item, index }"> </template>
        </ComFolioTransactionCreditDebitStyle>
      </Stack>
    </div>
  </ComDialogContent>
...
async function loadData() {
  ...
  getApi("city_ledger.get_balance_city_ledger_transaction", {
