<template>
  ...
  function loadData(showLoading = true) {
    ...
    localStorage.setItem("page_state_desk_folio_transaction", JSON.stringify(pageState.value))
  ...
  onMounted(() => {
    let state = localStorage.getItem("page_state_desk_folio_transaction")
    if (state) {
    ...
    window.socket.on("ComDeskFolioTransaction", (arg) => {
  ...
  onUnmounted(() => {
    window.socket.off("ComDeskFolioTransaction")
  })
