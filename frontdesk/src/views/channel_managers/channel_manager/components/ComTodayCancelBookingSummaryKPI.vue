<template>
    ...
    <div class="card cursor-pointer" @click="onOpenCancelledRes()">
        <div class="left">
            <div class="main-number">
                {{ data?.total_cancelled_reservations }}
            </div>

            <div class="title">
                {{ $t('Cancel Bookings Today') }}
            </div>

            <div class="subtitle">
                {{ data?.total_cancelled_reservations }} {{ $t('Stay Cancelled') }}
            </div>
        </div>
...
async function onOpenCancelledRes() {

    await onViewData(
        'Business%20Branch', 
        gv.getCustomPrintFormat("eDoor Cancel Reservation from CM"),
        [
...
async function onViewData(doctype, report_name, extra_params, filter_options) {
    ...
    await app.utils.openDialog(ComIFrameModal, "Channel Manager Cancel Booking Reservation", {
        data: {
            "doctype": doctype,
