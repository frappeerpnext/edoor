<template>
    <ComDialogContent @onOK="onOk" hideButtonClose titleButtonOK="Save" :hideIcon="false">
        ...
        <div>
            <Button label="Add Date Range" link @click="onAddDateRange" v-if="data.date_range.length < 12" />
        </div>
<Message severity="warn">If the value is left empty, the price will remain unchanged. If the price is set to 0, the price will be updated to 0.</Message>
        

<div>
    <Chip label="All Room Types" @click="onEnableUpdateAllRoomType()" :icon="(updateRoomTypes.size == roomTypes.length)?'pi pi-check':''"></Chip>
<Chip :label="rt.room_type_name"
 :icon="(updateRoomTypes.has(rt.edoor_room_type))?'pi pi-check':''"
 @click="onToggleRoomTypeToUpdate(rt.edoor_room_type)"
 
v-for="rt in roomTypes" :key="'rt_selection' + rt.edoor_room_type" />
        ...
        <table class="p-datatable-table">
            <thead>
                <tr>
                    ...
                    <th v-for="rt in data.room_types">
                        {{ rt.room_type_name }} <br/>
                        {{ rt.cm_room_type }}
                    </th>
                </tr>
            ...
            <tbody>
                <tr v-for="occ in occupancyCodes">
                    <td>{{ occ.title }}</td>
                    <td v-for="rt in data.room_types">
                       <template v-if="rt.occupancy_codes.find(x=>x.occupancy_code == occ.occupancy_code)">
                  
                        <ComInputCurrency  :disabled = "!updateRoomTypes.has(rt.edoor_room_type)" classCss="w-full" v-model="rt.occupancy_codes.find(x=>x.occupancy_code == occ.occupancy_code).rate" />
                       </template>
                    </td>
                </tr>
...
async function checkRoomRate(){
    ...
    const res = await app.postApi("rate_plan.check_room_rate_on_update",{data:{
