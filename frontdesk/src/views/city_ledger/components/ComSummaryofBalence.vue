<template>
    <div class="grid px-2  mb-3 mt-3">
                    <div :class="(index === summary.length - 1) ? 'bg-green-50 border-green-edoor' : 'bg-white'"
                        class="col-12 md:col md:mx-1 my-1 rounded-lg shadow-charge-total border"  v-for="(s, index) in summary"
                        :key="index">
                    ...
                    <ComBoxSummaryBalanceTransaction label="Transatction Durring" :value='moment(start_date).format("DD-MM-YYYY") + "  to  " + moment(end_date).format("DD-MM-YYYY")' :class="'bg-purple-50 border-purple-300 md:mx-1 my-1'"  />
                </div>
</template>
<script setup>
