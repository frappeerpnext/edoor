<template>
  <ComFrontDeskLayout :title="$t('Map View')" :showSetting="true" :showRefreshButton="true" @onRefresh="onRefreshData">
    <ComFilter @onFilter="onRefreshData" :filters="filters" :columns="data.columns" />
    <hr class="my-2">
    <Button @click="onSetting">Setting</Button>
    <div class="grid">
 ...
 <!-- summayr -->
    ...
    <TabView class="tabview-custom mt-3" @tab-change="onTabChange">
      ...
      <TabPanel :header="$t('Map View')">
        <ComViewDataOnMap :data="data.result" :columns="data.columns" :filters="filters" />
      </TabPanel>
      <TabPanel :header="$t('Data View')">

        <ComViewDataInTable v-if="tabTableViewLoaded" :data="data.result" :columns="data.columns" />
...
const filters = ref({ timespan: "This Year", display_field: "occupancy", row_group: "Nationality", property: property_name, chart_type: "bar", show_summary: 1, show_chart_series: [] });
...
async function onLoadData() {
  ...
  getApi(
    ...
    if (data.value.columns.filter(r=>r.fieldname ==filters.value.display_field).length==0){
